...
const TemplateSelector = ({selectedTemplate,onChange}) => {
    ...
    const templates=[
        ...
        },
        {
            id:"minimal",
            name:"Minimal",
            preview:"Ultra-clean design thats put your content front and center "
        },
    ]
  return (
    <div className='relative'>
        <button onClick={()=>setIsOpen(!isOpen)} className='flex items-center gap-1 
        text-sm text-blue-600 bg-gradient-to-br from-blue-50
         to-blue-100 ring-blue-300 hover:ring transition-all 
            ...
            bg-white rounded-md border border-gray-200 shadow-sm'>
                {templates.map((template)=>(
                    <div key={template.id} onClick={()=>{onChange(template.id);
                        setIsOpen(false)}} className={`relative p-3 rounded-md
                       ...
                       <div>
                        ...
                        text-xs text-gray-500 italic '>{template.preview}</div>
                       </div>
                    </div>
                ))}
            </div>
