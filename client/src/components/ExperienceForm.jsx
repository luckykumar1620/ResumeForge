...
const ExperienceForm = ({data,onChange}) => {
  ...
  const [generatingIndex,setGeneratingIndex]=useState(-1)
    ...
    const generateDescription=async(index)=>{
      ...
      const prompt=`enhance this job description ${experience.description} for the position of 
    ...
    <div className='space-y-6'>
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-sm text-gray-500'>Add your job 
                Experience
            </p>
          </div>
          <button onClick={addExperience} className='flex items-center gap-2 px-3 py-1 text-sm
          bg-green-100 text-shadow-green-700 rounded-lg hover:bg-green-200
          transition-colors '>
                    ...
                    rounded-lg space-y-3'>
                      ...
                      <label className='flex items-center gap-2'>
                        <input type="checkbox" checked={experience.is_current ||
                          false} onChange={(e)=>{updateExperience(index,"is_current",
                            e.target.checked?true:false);}} className='rounded border-gray-300
                      ...
                      <div className='space-y-2'>
                        <div className='flex items-center justify-between'>
                            ...
                            disabled={generatingIndex===index||!experience.position||!experience.company} className='flex items-center gap-1 px-2 py-1 text-xs
                        ...
                        <textarea value={experience.description||""} onChange=
                        {(e)=>updateExperience(index,"description",e.target.value)}
                        rows={4} className='w-full text-sm px-3 py-2 rounded-lg 
                        resize-none ' placeholder='Describe your key responsibilities and achievements...' />
                      </div>
                    </div>
                ))}
            </div>
